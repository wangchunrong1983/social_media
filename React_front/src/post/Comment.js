...
import DefaultProfile from "../images/userIcon.jpg";
...
class Comment extends Component {
    ...
    addComment = e => {
        e.preventDefault();
        if (!isAuthenticated()) {
            this.setState({ error: "Please signin to leave comments" });
            return false;
        }
        if (this.isValid()) {
    ...
    deleteConfirmed = comment => {
        let answer = window.confirm("To delete this comment?");
        if (answer) {
                        ...
                        <div key={i}>
                            <div>
                                <Link to={`/user/${comment.postedBy._id}`}>
                                    <img style={{borderRadius: "50%", border: "1px solid black"}} className="float-left mr-2"
                                        height="30px" width="30px" onError={i =>(i.target.src = `${DefaultProfile}`)}
